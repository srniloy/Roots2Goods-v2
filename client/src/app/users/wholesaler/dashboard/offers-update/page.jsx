...
import { AddOrders, GetOffersList, OfferCancellation } from '@services/td-service/product_service';
...
const OfferUpdates = (props) => {
    ...
    const proceedToBilling = async (row) =>{
      const data = {
        seller_id: row.project_id.created_by._id,
        buyer_id: user?._id,
        sales_id: row.sales_id,
        product_id: row.project_id._id,
        offer_id: row._id
        }
                            ...
                            {columns.map((column) => {
                                if(column.id == 'cancel'){
                                    ...
                                    return(
                                        ...
                                        <Button disabled={row['status'] == 'Accepted' ? false : true} color='primary' startIcon={<AutorenewIcon />} variant='contained' onClick={() =>{
                                ...
                                // }
                                    ...
                                    const value = row.project_id.created_by[column.id];
                                    return (
                                    <TableCell key={column.id} align={column.align}>
                                        {column.format && typeof value === 'number'
                                        ? column.format(value)
